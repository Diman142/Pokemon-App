    ...
    <div className="container pt-4">
      ...
      <Form className="pokemon-list-form">
        <Form.Group controlId="formBasicEmail">
          ...
          <Form.Control type="search" value={searchValue} onChange={(event) => {
        ...
        <Button variant="primary" type="submit" className="mb-4" onClick={(event) => {
      ...
      {isLoaded ?
        <ul className="pokemon-list">
          {pokemonCurrentList.sort().map((item, index) => {
            return <li className="pokemon-list__item" key={index}>  <Link className="pokemon-list__link" to={"/" + item} onClick={(event) => {
